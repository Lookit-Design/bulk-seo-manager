 ...
 * Requires PHP: 7.4
 * Text Domain:  lookit-bulk-seo-manager
 */
...
function bsm_ajax_get_meta_fields(): void {
	...
	try {
		global $wpdb;
		$all_keys = wp_cache_get( 'bsm_distinct_meta_keys', 'lookit-bulk-seo-manager' );
		if ( false === $all_keys ) {
			...
			wp_cache_set( 'bsm_distinct_meta_keys', $all_keys, 'lookit-bulk-seo-manager', 5 * MINUTE_IN_SECONDS );
		}
	...
	<div class="wrap" id="bsm-wrap">
		...
		<form method="post" action="options.php" id="bsm-settings-form">
			...
			<div class="bsm-set-shell" id="bsm-set-shell">
				...
				<?php /* ── Nav rail ── */ ?>
				<nav class="bsm-set-rail" id="bsm-set-rail" aria-label="<?php esc_attr_e( 'Settings sections', 'lookit-bulk-seo-manager' ); ?>">
					<?php
				...
				<div class="bsm-set-main">
					...
					<section class="bsm-set-pane" data-pane="textsize">
						...
						<div class="bsm-set-sizes" role="radiogroup" aria-label="<?php esc_attr_e( 'Text size', 'lookit-bulk-seo-manager' ); ?>">
							<?php
					...
					<section class="bsm-set-pane" data-pane="test">
						...
						<div class="bsm-set-inline">
							...
							<button type="button" id="asy-ai-btn" class="button bsm-set-ai"
									title="<?php esc_attr_e( 'Generate focus keyphrase, related keyphrases, and meta description with Amazon Bedrock, and save them to Yoast', 'lookit-bulk-seo-manager' ); ?>">
								&#10022; Use AI
							</button>
						</div>
						<p class="bsm-set-hint">
					...
					<section class="bsm-set-pane" data-pane="help">
						...
						<ul class="bsm-set-help">
							<li><?php esc_html_e( 'Bulk Editor fills the Yoast focus keyphrase, meta description, and related keyphrases for the posts you choose; Auto SEO Manager does the same automatically when a post is published.', 'lookit-bulk-seo-manager' ); ?></li>
							<li><?php esc_html_e( 'Keyphrase source: "Full title", "Slug", their first-3-words variants, "Top content word" (strongest phrase in the content), or AI — Nova Lite via the platform.', 'lookit-bulk-seo-manager' ); ?></li>
							<li><?php esc_html_e( 'Related keyphrases: Off, Datamuse (free, no key), or AI — Nova Lite. Meta description: a saved template, AI — Nova Lite, or none.', 'lookit-bulk-seo-manager' ); ?></li>
							<li><?php esc_html_e( 'Any AI option is generated with Amazon Bedrock through the platform (set the endpoint under AI engine). If the platform is unreachable, Auto SEO falls back to the post title so publishing never breaks.', 'lookit-bulk-seo-manager' ); ?></li>
							<li><?php esc_html_e( 'Auto SEO overwrites the fields on each publish. Requires Yoast SEO (free or premium).', 'lookit-bulk-seo-manager' ); ?></li>
						</ul>
					</section>
				...
				<aside class="bsm-set-prev" id="bsm-set-prev" aria-live="polite">
					...
					<div class="bsm-set-sample">
						<p class="bsm-set-sample-t" id="bsm-sample-title">
							<?php echo $bsm_sample ? esc_html( $bsm_sample->post_title ) : esc_html__( 'No posts found', 'lookit-bulk-seo-manager' ); ?>
						</p>
	...
	if ( 'auto' === $tab && current_user_can( 'manage_options' ) ) {
		...
		wp_localize_script(
			...
			array(
				...
				'saved'       => __( 'Settings saved!', 'lookit-bulk-seo-manager' ),
				'key_saved'   => __( 'API key saved!', 'lookit-bulk-seo-manager' ),
				'error'       => __( 'Save failed. Please try again.', 'lookit-bulk-seo-manager' ),
...
function bsm_render_page(): void {
	...
	<div class="wrap" id="bsm-wrap">
		...
		if ( $saved > 0 ) :
			...
			printf( esc_html( _n( '%d item saved.', '%d items saved.', $saved, 'lookit-bulk-seo-manager' ) ), (int) $saved );
			?>
			</p></div><?php endif; ?>
		<?php
		if ( $errors > 0 ) :
			...
			printf( esc_html__( '%d item(s) could not be updated.', 'lookit-bulk-seo-manager' ), (int) $errors );
			?>
			</p></div><?php endif; ?>
		...
		<p class="bsm-cpt-count">
			...
			printf( esc_html__( '%1$d post types: %2$s', 'lookit-bulk-seo-manager' ), count( $all_types ), esc_html( implode( ', ', array_map( fn( $t ) => $t->label, $all_types ) ) ) );
		...
		<form method="post" id="bsm-bulk-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<div class="bsm-bottom-bar">
				<span style="font-size:calc(12px * var(--bsm-fs, 1));color:#666;">
					...
					printf( esc_html__( 'Showing %1$d–%2$d of %3$d', 'lookit-bulk-seo-manager' ), (int) ( ( ( $paged - 1 ) * BSM_PER_PAGE ) + 1 ), (int) min( $paged * BSM_PER_PAGE, $total ), (int) $total );
					?>
				</span>
