<?php
/**
 * Plugin Name:  Lookit Bulk SEO Manager
...
function bsm_get_post_types(): array {
    ...
    foreach ( $exclude as $slug ) unset( $types[ $slug ] );
    return $types;
}
...
function bsm_register_menu() {
    add_menu_page( 'Lookit Bulk SEO Manager', 'Bulk SEO', 'edit_posts', 'lookit-bulk-seo', 'bsm_render_page', 'dashicons-chart-line', 58 );
    add_submenu_page( 'lookit-bulk-seo', 'Lookit Bulk SEO Manager', 'Bulk SEO Manager',  'edit_posts',      'lookit-bulk-seo',          'bsm_render_page' );
    add_submenu_page( 'lookit-bulk-seo', 'Bulk SEO Settings',       'Bulk SEO Settings', 'manage_options',  'lookit-bulk-seo-settings', 'bsm_render_settings' );
}

// ─── Settings registration ────────────────────────────────────────────────────

add_action( 'admin_init', function() {
    register_setting( 'bsm_settings_group', 'bsm_custom_templates', [
        'sanitize_callback' => 'bsm_sanitize_templates',
        'default'           => [],
    ] );
    register_setting( 'bsm_settings_group', 'bsm_keyphrase_templates', [
        'sanitize_callback' => 'bsm_sanitize_templates',
        'default'           => [],
    ] );
    register_setting( 'bsm_settings_group', 'bsm_title_templates', [
        'sanitize_callback' => 'bsm_sanitize_templates',
        'default'           => [],
    ] );
} );
...
function bsm_sanitize_templates( $input ): array {
    ...
    foreach ( $input as $item ) {
        ...
        if ( $label !== '' && $template !== '' ) {
            $out[] = [ 'label' => $label, 'template' => $template ];
        }
    }
    return $out;
}
...
function bsm_get_templates(): array {
    return (array) get_option( 'bsm_custom_templates', [] );
}

function bsm_get_keyphrase_templates(): array {
    return (array) get_option( 'bsm_keyphrase_templates', [] );
}
...
function bsm_text_sizes(): array {
    return [
        'default' => [ 'label' => 'Default',     'scale' => '1',    'note' => 'Matches WordPress admin' ],
        'large'   => [ 'label' => 'Large',       'scale' => '1.1',  'note' => '10% larger' ],
        'xlarge'  => [ 'label' => 'Larger',      'scale' => '1.2',  'note' => '20% larger' ],
        'xxlarge' => [ 'label' => 'Largest',     'scale' => '1.35', 'note' => '35% larger' ],
    ];
}
...
function bsm_ajax_save_text_size(): void {
    ...
    $sizes = bsm_text_sizes();
    if ( ! isset( $sizes[ $key ] ) ) wp_send_json_error( 'Unknown size.' );

    update_user_meta( get_current_user_id(), 'bsm_text_size', $key );
    wp_send_json_success( [ 'scale' => $sizes[ $key ]['scale'] ] );
}


function bsm_get_title_templates(): array {
    return (array) get_option( 'bsm_title_templates', [] );
}
...
function bsm_resolve_template( string $template, WP_Post $post ): string {
    ...
    $out = preg_replace_callback( '/\{([a-zA-Z0-9_\-]+)\}/', function ( $m ) use ( $post ) {
...
function bsm_ajax_get_meta_fields(): void {
    ...
    try {
        if ( function_exists( 'jet_engine' ) && isset( jet_engine()->meta_boxes ) ) {
            ...
            foreach ( (array) $meta_boxes as $box ) {
                ...
                foreach ( (array) ( $box['meta_fields'] ?? [] ) as $field ) {
                    $name  = $field['name']  ?? '';
    ...
    try {
        if ( function_exists( 'acf_get_field_groups' ) ) {
            ...
            foreach ( (array) $groups as $group ) {
                ...
                $gfields = function_exists( 'acf_get_fields' ) ? acf_get_fields( $group['key'] ?? '' ) : [];
    ...
    $core = [
        [ 'key' => '_title',     'label' => 'Post Title',            'source' => 'WordPress', 'type' => 'text',     'placeholder' => '{title}' ],
        [ 'key' => '_excerpt',   'label' => 'Post Excerpt / Content','source' => 'WordPress', 'type' => 'textarea', 'placeholder' => '{excerpt}' ],
        [ 'key' => '_type',      'label' => 'Post Type Label',       'source' => 'WordPress', 'type' => 'text',     'placeholder' => '{type}' ],
        [ 'key' => '_site',      'label' => 'Site Name',             'source' => 'WordPress', 'type' => 'text',     'placeholder' => '{site}' ],
        [ 'key' => '_category',  'label' => 'First Category / Term', 'source' => 'WordPress', 'type' => 'text',     'placeholder' => '{category}' ],
        [ 'key' => '_keyphrase', 'label' => 'Yoast Focus Keyphrase', 'source' => 'Yoast',     'type' => 'text',     'placeholder' => '{keyphrase}' ],
    ];
    ...
    try {
        ...
        $skip_prefixes  = [ '_', 'seopress', 'rankmath', 'rank_math', 'wp_', 'elementor', '_elementor' ];
        $skip_contains  = [ 'transient', 'session', 'nonce', 'token', 'password', 'hash' ];
        $already_keys   = array_column( $fields, 'key' );

        foreach ( (array) $all_keys as $mk ) {
            if ( in_array( $mk, $already_keys, true ) ) continue;

            // Skip private/internal keys
            $skip = false;
            foreach ( $skip_prefixes as $prefix ) {
                if ( strpos( $mk, $prefix ) === 0 ) { $skip = true; break; }
            }
            if ( ! $skip ) {
                foreach ( $skip_contains as $needle ) {
                    if ( strpos( strtolower($mk), $needle ) !== false ) { $skip = true; break; }
                }
            }
            if ( $skip ) continue;
    ...
    wp_send_json_success( [ 'core' => $core, 'fields' => $fields ] );
}
...
function bsm_render_settings() {
    ...
    <div class="wrap" id="bsm-wrap">
        ...
        if ( ! $bsm_sample_id || ! get_post( $bsm_sample_id ) ) {
            $bsm_recent    = get_posts( [ 'numberposts' => 1, 'post_status' => 'publish', 'orderby' => 'modified' ] );
            $bsm_sample_id = $bsm_recent ? (int) $bsm_recent[0]->ID : 0;
        }
        $bsm_sample = $bsm_sample_id ? get_post( $bsm_sample_id ) : null;

        $bsm_panes = [
            'engine'   => [ 'Setup',     'AI engine' ],
            'defaults' => [ 'Setup',     'Defaults' ],
            'textsize' => [ 'Setup',     'Text size' ],
            'title'    => [ 'Templates', 'SEO titles' ],
            'desc'     => [ 'Templates', 'Descriptions' ],
            'kp'       => [ 'Templates', 'Keyphrases' ],
            'test'     => [ 'Tools',     'Test &amp; reprocess' ],
            'fields'   => [ 'Tools',     'Custom fields' ],
            'help'     => [ 'Tools',     'How it works' ],
        ];
        $bsm_counts = [
        ...
        <form method="post" action="options.php" id="bsm-settings-form">
            ...
            <div class="bsm-set-shell" id="bsm-set-shell">
                ...
                <nav class="bsm-set-rail" id="bsm-set-rail" aria-label="<?php esc_attr_e( 'Settings sections', 'bulk-keyphrase-manager' ); ?>">
                    <?php $bsm_last_group = ''; foreach ( $bsm_panes as $bsm_key => $bsm_meta ) :
                ...
                <div class="bsm-set-main">
                    ...
                    $bsm_tpl_panes = [
                        ...
                        'desc'  => [
                            ...
                            'chips' => [
                                [ '{excerpt}', 'First 25 words of content' ],
                                [ '{title}', 'Page title' ],
                                [ '{keyphrase}', 'Yoast focus keyphrase' ],
                                [ '{category}', 'First category / term' ],
                                [ '{type}', 'Post type label' ],
                                [ '{site}', 'Site name' ],
                                [ '{sep}', 'Separator (–)' ],
                    ...
                    <section class="bsm-set-pane" data-pane="textsize">
                        ...
                        <div class="bsm-set-sizes" role="radiogroup" aria-label="<?php esc_attr_e( 'Text size', 'bulk-keyphrase-manager' ); ?>">
                            <?php $bsm_current_size = bsm_get_text_size();
                ...
                <aside class="bsm-set-prev" id="bsm-set-prev" aria-live="polite">
                    ...
                    <div class="bsm-set-sample">
                        ...
                        <p class="bsm-set-sample-m">
                            <span class="bsm-set-tag" id="bsm-sample-type"><?php
                    ...
                    <div class="bsm-set-serp">
                        <div class="bsm-set-serp-url" id="bsm-serp-url"><?php
...
function bsm_ajax_ai_fill() {
    ...
    $site    = array( 'url' => home_url(), 'name' => get_bloginfo( 'name' ) );
    $results = array();
    $errors  = array();
    ...
    foreach ( $ids as $id ) {
        ...
        if ( ! $terms || is_wp_error( $terms ) ) {
            foreach ( get_object_taxonomies( $post->post_type ) as $tax ) {
                ...
                if ( $tt && ! is_wp_error( $tt ) ) { $terms = $tt; break; }
            }
        }
        if ( $terms && ! is_wp_error( $terms ) ) {
 ...
 */
function bsm_ai_call_webhook( $task, WP_Post $post, $count = 3, $keyphrase = '' ) {
    $webhook = trim( (string) get_option( 'bsm_ai_webhook_url', '' ) );
    if ( empty( $webhook ) ) {
    ...
    foreach ( $ids as $id ) {
        ...
        if ( 'datamuse' === $source ) {
            if ( class_exists( 'ASY_Keyphrase_Engine' ) ) {
                ...
                if ( is_wp_error( $r ) ) { $errors[ $id ] = $r->get_error_message(); continue; }
                $related = is_array( $r ) ? $r : array();
            }
        } else { // ai
            $list = bsm_ai_call_webhook( 'keyphrase', $post, $count + 1 );
            if ( is_wp_error( $list ) ) { $errors[ $id ] = $list->get_error_message(); continue; }
        ...
        if ( ! empty( $related ) && class_exists( 'ASY_Keyphrase_Engine' ) ) {
            ...
            if ( is_array( $decoded ) ) {
                $results[ $id ] = array_values( array_filter( array_map( function ( $o ) {
...
function bsm_handle_save() {
    ...
    if ( ! isset( $_POST['bsm_action'] ) || sanitize_key( wp_unslash( $_POST['bsm_action'] ) ) !== 'save_all' ) return;
    if ( ! check_admin_referer( BSM_NONCE ) ) return;
    ...
    if ( empty( $page_ids ) ) {
        $page_ids = array_unique( array_merge(
...
function bsm_update_fields( int $post_id, string $keyphrase, string $metadesc, string $title = '' ): void {
    if ( $keyphrase !== '' ) update_post_meta( $post_id, BSM_META_KW,    $keyphrase );
    if ( $metadesc  !== '' ) update_post_meta( $post_id, BSM_META_DESC,  $metadesc );
    if ( $title     !== '' ) update_post_meta( $post_id, BSM_META_TITLE, $title );
    if ( ( $keyphrase !== '' || $metadesc !== '' || $title !== '' ) && class_exists( '\Yoast\WP\SEO\Models\Indexable' ) ) {
        try {
            ...
            if ( $repo ) {
                ...
                if ( $idx ) {
                    if ( $keyphrase !== '' ) $idx->primary_focus_keyword = $keyphrase;
                    if ( $metadesc  !== '' ) $idx->description           = $metadesc;
                    if ( $title     !== '' ) $idx->title                 = $title;
                    $idx->save();
                }
            }
...
function bsm_topbar(): void {
    ...
    <div class="bsm-topbar">
        ...
        <div>
            <span class="bsm-topbar-title">Lookit Bulk SEO Manager</span>
        ...
        <?php if ( $jet ) : ?><span class="bsm-badge bsm-badge-jet">JetEngine</span><?php endif; ?>
...
function bsm_enqueue_assets( $hook ): void {
    ...
    if ( in_array( $hook, [ 'post.php', 'post-new.php' ], true ) ) {
        wp_enqueue_style( 'lookit-bsm-admin', ASY_PLUGIN_URL . 'assets/admin.css', [], ASY_VERSION );
        return;
    }

    if ( strpos( (string) $hook, 'lookit-bulk-seo' ) === false ) return;
    wp_enqueue_style( 'google-dm-sans', 'https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&display=swap', [], BSM_VERSION );
    ...
    if ( 'settings' === $bsm_tab || strpos( (string) $hook, 'lookit-bulk-seo-settings' ) !== false ) {
        ...
        if ( '' !== $bsm_set_css ) {
            ...
            wp_enqueue_style( 'bsm-settings', ASY_PLUGIN_URL . 'assets/settings.css', [], BSM_VERSION );
        }

        wp_enqueue_script( 'bsm-settings', ASY_PLUGIN_URL . 'assets/settings.js', [], BSM_VERSION, true );
        wp_localize_script( 'bsm-settings', 'BSM_SET', [
    ...
    if ( $tab === 'auto' && current_user_can( 'manage_options' ) ) {
        wp_enqueue_style(  'lookit-bsm-admin', ASY_PLUGIN_URL . 'assets/admin.css', [], ASY_VERSION );
        wp_enqueue_script( 'lookit-bsm-admin', ASY_PLUGIN_URL . 'assets/admin.js', [ 'jquery' ], ASY_VERSION, true );
        wp_localize_script( 'lookit-bsm-admin', 'ASY', [
            'ajax_url'    => admin_url( 'admin-ajax.php' ),
...
function bsm_render_page(): void {
    ...
    if ( $tab === 'auto' && current_user_can( 'manage_options' ) ) { bsm_render_auto_section(); return; }
    if ( $tab === 'settings' && current_user_can( 'manage_options' ) ) { bsm_render_settings(); return; }
    ...
    $bsm_statuses = [ 'publish', 'draft', 'pending', 'future', 'private', 'any' ];
    if ( ! in_array( $fpost, $bsm_statuses, true ) ) $fpost = 'publish';
    ...
    if ( $ftype !== 'all' && ! isset( $all_types[ $ftype ] ) ) $ftype = 'all';

    $qargs = [
        'post_type'      => $ftype === 'all' ? array_keys( $all_types ) : [ $ftype ],
        'post_status'    => $fpost,
    ...
    if ( $fstatus === 'set' ) {
        $qargs['meta_query'] = [ [ 'key' => BSM_META_KW, 'value' => '', 'compare' => '!=' ] ];
    } elseif ( $fstatus === 'empty' ) {
        $qargs['meta_query'] = [ 'relation' => 'OR',
            [ 'key' => BSM_META_KW, 'compare' => 'NOT EXISTS' ],
            [ 'key' => BSM_META_KW, 'value' => '', 'compare' => '=' ],
        ];
    } elseif ( $fstatus === 'duplicate' ) {
        $dup_ids = bsm_duplicate_keyphrase_post_ids(
            $ftype === 'all' ? array_keys( $all_types ) : [ $ftype ]
        );
        $qargs['post__in'] = ! empty( $dup_ids ) ? $dup_ids : [ 0 ];
    ...
    if ( $fdesc === 'set' ) {
        $desc_clause = [ [ 'key' => BSM_META_DESC, 'value' => '', 'compare' => '!=' ] ];
    } elseif ( $fdesc === 'empty' ) {
        $desc_clause = [ 'relation' => 'OR',
            [ 'key' => BSM_META_DESC, 'compare' => 'NOT EXISTS' ],
            [ 'key' => BSM_META_DESC, 'value' => '', 'compare' => '=' ],
        ];
    } else {
        $desc_clause = [];
    }
    if ( ! empty( $desc_clause ) ) {
        if ( ! empty( $qargs['meta_query'] ) ) {
            $qargs['meta_query'] = [ 'relation' => 'AND', $qargs['meta_query'], $desc_clause ];
        } else {
    ...
    $jet_count  = 0;
    if ( $jet_active ) foreach ( array_keys( $all_types ) as $s ) { if ( ! in_array( $s, ['post','page'], true ) ) $jet_count++; }
    ?>
    <div class="wrap" id="bsm-wrap">
        <?php bsm_topbar(); ?>
        <?php bsm_render_tabs( 'bulk' ); ?>

        <?php if ( $saved > 0 ) : ?><div class="notice notice-success is-dismissible"><p><?php
            /* translators: %d: number of items saved. */
            printf( esc_html( _n( '%d item saved.', '%d items saved.', $saved, 'bulk-keyphrase-manager' ) ), (int) $saved ); ?></p></div><?php endif; ?>
        <?php if ( $errors > 0 ) : ?><div class="notice notice-warning is-dismissible"><p><?php
            /* translators: %d: number of items that could not be updated. */
            printf( esc_html__( '%d item(s) could not be updated.', 'bulk-keyphrase-manager' ), (int) $errors ); ?></p></div><?php endif; ?>
        ...
        <form method="post" id="bsm-bulk-form" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
            ...
            <div class="bsm-bottom-bar">
                ...
                <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                    <?php echo wp_kses_post( paginate_links( ['base'=>add_query_arg('paged','%#%'),'format'=>'','current'=>$paged,'total'=>$total_pages,'prev_text'=>'&laquo;','next_text'=>'&raquo;'] ) ); ?>
