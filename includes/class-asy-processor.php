...
class ASY_Processor {
	...
	public function register_lock_metabox() {
		...
		foreach ( $post_types as $pt ) {
			add_meta_box(
				'asy_seo_lock',
				__( 'Auto SEO', 'lookit-bulk-seo-manager' ),
		...
		<div class="asy-lock-wrap">
			<label class="asy-lock-label">
				<input type="checkbox"
						...
						value="1"
						<?php checked( $locked ); ?>>
				<span><?php esc_html_e( 'Lock SEO fields', 'lookit-bulk-seo-manager' ); ?></span>
			</label>
			<p class="asy-lock-hint">
				<?php esc_html_e( 'When locked, Auto SEO will not overwrite the focus keyphrase, meta description, or related keyphrases on publish.', 'lookit-bulk-seo-manager' ); ?>
			</p>
			<?php if ( $locked ) : ?>
				<p class="asy-lock-status asy-lock-status--on">
					🔒 <?php esc_html_e( 'SEO fields are protected', 'lookit-bulk-seo-manager' ); ?>
				</p>
			<?php else : ?>
				<p class="asy-lock-status asy-lock-status--off">
					🔓 <?php esc_html_e( 'Auto SEO is active', 'lookit-bulk-seo-manager' ); ?>
				</p>
			<?php endif; ?>
		</div>
		<?php
