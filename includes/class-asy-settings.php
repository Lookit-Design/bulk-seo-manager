...
class ASY_Settings {
	...
	public function register_menu() {
		...
		add_submenu_page(
			$parent,
			__( 'Auto SEO Templates', 'lookit-bulk-seo-manager' ),
			__( 'Auto SEO Templates', 'lookit-bulk-seo-manager' ),
			'manage_options',
	...
	public function enqueue_assets( $hook ) {
		...
		if ( strpos( $hook, 'lookit-bulk-seo-manager' ) !== false ) {
	...
	public function ajax_save_templates() {
		...
		if ( empty( $raw ) ) {
			wp_send_json_error( __( 'No settings arrived at the server — nothing was saved.', 'lookit-bulk-seo-manager' ) );
		}
		...
		if ( $expected > 0 && count( $raw ) < $expected ) {
			wp_send_json_error(
				...
				/* translators: 1: rows received, 2: rows sent */
					__( 'Only %1$d of %2$d rows reached the server — PHP dropped part of the request (max_input_vars). Nothing was saved.', 'lookit-bulk-seo-manager' ),
					count( $raw ),
					$expected
				)
			);
	...
	public function render_page( $embedded = false ) {
		...
		$placeholders = array(
			'{title}'     => __( 'Page title', 'lookit-bulk-seo-manager' ),
			'{site}'      => __( 'Site name', 'lookit-bulk-seo-manager' ),
			'{keyphrase}' => __( 'Post title used as keyphrase', 'lookit-bulk-seo-manager' ),
			'{excerpt}'   => __( 'First 25 words of content', 'lookit-bulk-seo-manager' ),
			'{category}'  => __( 'First category / term', 'lookit-bulk-seo-manager' ),
			'{type}'      => __( 'Post type label', 'lookit-bulk-seo-manager' ),
		);
		?>
		<div class="<?php echo $embedded ? 'asy-wrap asy-embedded' : 'wrap asy-wrap'; ?>">
			...
			<div class="asy-header">
				<?php if ( $embedded ) : ?>
					<h2 style="font-family:'DM Sans',system-ui,sans-serif;font-size:calc(18px * var(--bsm-fs, 1));margin:0 0 6px;"><?php esc_html_e( 'Auto SEO Manager', 'lookit-bulk-seo-manager' ); ?></h2>
				<?php else : ?>
					<h1><?php esc_html_e( 'Auto SEO Templates', 'lookit-bulk-seo-manager' ); ?></h1>
				<?php endif; ?>
				<p class="asy-subtitle">
					<?php esc_html_e( 'Auto-fill Yoast keyphrase, meta description, related keyphrases, and SEO title when a post is published. Add templates in Settings → Description Templates.', 'lookit-bulk-seo-manager' ); ?>
				</p>
			</div>
			...
			<table class="bsm-table widefat bsm-auto-table">
				<thead>
					<tr>
						<th class="bsm-auto-th-toggle"><?php esc_html_e( 'Enable', 'lookit-bulk-seo-manager' ); ?></th>
						<th><?php esc_html_e( 'Post Type', 'lookit-bulk-seo-manager' ); ?></th>
						<th><?php esc_html_e( 'Auto Keyphrase', 'lookit-bulk-seo-manager' ); ?></th>
						<th><?php esc_html_e( 'Related Keyphrases', 'lookit-bulk-seo-manager' ); ?></th>
						<th><?php esc_html_e( 'Meta Description Template', 'lookit-bulk-seo-manager' ); ?></th>
						<th><?php esc_html_e( 'SEO Title', 'lookit-bulk-seo-manager' ); ?></th>
					</tr>
				</thead>
					...
					<tr class="asy-row<?php echo $enabled ? ' asy-row--active' : ''; ?>" data-pt="<?php echo esc_attr( $pt_slug ); ?>">
						...
						<td class="asy-col-tpl">
							...
							<select class="asy-tpl-input asy-select"
								...
								<option value=""><?php esc_html_e( '— No meta description —', 'lookit-bulk-seo-manager' ); ?></option>
								<optgroup label="── AI (Amazon Bedrock) ──">
								...
								<?php if ( '' !== (string) $tpl && ! $tpl_matched ) : ?>
									<option value="<?php echo esc_attr( $tpl ); ?>" selected>
										<?php echo esc_html( $tpl ); ?> <?php esc_html_e( '(custom)', 'lookit-bulk-seo-manager' ); ?>
									</option>
								<?php endif; ?>
							</select>
						</td>
						...
						<td class="asy-col-title">
							<select class="asy-title-source asy-select" name="templates[<?php echo esc_attr( $pt_slug ); ?>][title_source]">
								<option value=""><?php esc_html_e( '— No SEO title —', 'lookit-bulk-seo-manager' ); ?></option>
								<optgroup label="── AI (Amazon Bedrock) ──">
								...
								<?php if ( '' !== (string) $title_src && ! $title_matched ) : ?>
									<option value="<?php echo esc_attr( $title_src ); ?>" selected><?php echo esc_html( $title_src ); ?> <?php esc_html_e( '(custom)', 'lookit-bulk-seo-manager' ); ?></option>
								<?php endif; ?>
							</select>
						</td>
			...
			<p class="bsm-auto-save">
				<button type="button" id="asy-save-btn" class="button button-primary">
					<?php esc_html_e( 'Save Settings', 'lookit-bulk-seo-manager' ); ?>
				</button>
				<span id="asy-save-state" class="asy-save-state" data-rows="<?php echo esc_attr( (string) count( $post_types ) ); ?>">
					<?php esc_html_e( 'All changes saved', 'lookit-bulk-seo-manager' ); ?>
				</span>
			</p>
